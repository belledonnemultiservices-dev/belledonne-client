...
// ── ÉDITION EN LIGNE DES RAPPORTS EXCEL (gestion-rapports.html) ───
// storagePath + token de téléchargement sont encodés dans fileUrl (voir
// receiveKizeoSubmission) : on les en extrait plutôt que de les stocker en double.
function parseFileUrl(fileUrl) {
  const m = fileUrl.match(/\/o\/([^?]+)\?alt=media&token=([\w-]+)/);
  if (!m) throw new Error("fileUrl invalide");
...
async function adminEmails(db) {
  const emails = new Set(["belledonne.multiservices@gmail.com"]);
  try {
    const snap = await db.collection("users").where("role", "in", ["admin", "administrateur"]).get();
    snap.forEach(d => { if (d.data().email) emails.add(d.data().email); });
  } catch(e) { /* on garde au moins le compte maître */ }
  return Array.from(emails);
}

// Convertit le rapport Excel stocké en Google Sheet temporaire, partagé en édition
...
exports.openReportForEdit = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      const { getFirestore } = require("firebase-admin/firestore");
      const db = getFirestore(admin.app(), "belledonne-client");
      const ref = db.collection("reception-rapports").doc(docId);
      const snap = await ref.get();
      if (!snap.exists) { res.status(404).json({ error: "Rapport introuvable" }); return; }
      const r = snap.data();
      if (r.type !== "excel") { res.status(400).json({ error: "Édition en ligne réservée aux rapports Excel" }); return; }

      const { storagePath } = parseFileUrl(r.fileUrl);
      ...
      await Promise.all(emails.map(email =>
        ...
        }).catch(e => console.error(`openReportForEdit: partage échoué pour ${email}:`, e.message))
      ));

      const editUrl = `https://docs.google.com/spreadsheets/d/${fileId}/edit`;
      await ref.update({ gsheetId: fileId, gsheetUrl: editUrl });
      res.status(200).json({ fileId, editUrl });
    } catch(err) {
      console.error("openReportForEdit:", err.message);
      res.status(500).json({ error: err.message });
    }
  });

// Exporte le Google Sheet modifié en .xlsx, écrase le fichier d'origine dans
// Storage (même storagePath, même token de téléchargement) et supprime le
...
exports.saveReportEdit = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      const bucket = admin.storage().bucket();
      await bucket.file(storagePath).save(Buffer.from(exported.data), {
        contentType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        metadata: { metadata: { firebaseStorageDownloadTokens: token } },
      });

      await drive.files.delete({ fileId }).catch(e => console.error("saveReportEdit: suppression Drive échouée:", e.message));
      await ref.update({ gsheetId: null, gsheetUrl: null, updatedAt: new Date().toISOString() });

      res.status(200).json({ success: true });
    } catch(err) {
      console.error("saveReportEdit:", err.message);
      res.status(500).json({ error: err.message });
    }
  });
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      if (docId) {
        const { getFirestore } = require("firebase-admin/firestore");
        const db = getFirestore(admin.app(), "belledonne-client");
        await db.collection("reception-rapports").doc(docId).update({ gsheetId: null, gsheetUrl: null }).catch(() => {});
      }
      res.status(200).json({ success: true });
    } catch(err) {
      console.error("discardReportEdit:", err.message);
      res.status(500).json({ error: err.message });
    }
  });
